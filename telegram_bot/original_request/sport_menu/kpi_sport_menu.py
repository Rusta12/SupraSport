...
def sum_row(df):
    ...
    sum_row['Учреждение'] = 'Итого по виду спорта'
    df = pd.concat([df, pd.DataFrame([sum_row])], ignore_index=True)
    return df


def get_sport_kpi(message, df):
    df = sum_row(df)
    ...
    # Настройки
    plt.yticks(df.index, df['Учреждение'], fontsize=14)
    plt.title('Изменение общей численности за год.', fontdict={'size':20, 'fontproperties': font_prop}, pad=25)
    ...
    #plt.show()
    image_name = f'./gosorder/reporter_gosorder/Temp/sport_kpi_{message.chat.id}.png'
