...
def profile_user_print(message):
    ...
    supralogin = df.loc[0, 'supralogin']
    suprapass = df.loc[0,'suprapass']
    bot.send_message(
        message.chat.id,
        text=f'<b>Ваш уровень доступа -</b> <i>{role}!</i>'
                ...
                f'\n\nЧто бы попасть в аналитику перейдите по адресу http://172.17.30.4 (только для внутренний сети)'
                f'\n<tg-spoiler>Ваш логин: {supralogin} | ваш пароль: {suprapass} </tg-spoiler>\n'
